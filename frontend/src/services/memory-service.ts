...
export const searchVideos = async (
	...
	const response = await fetch(`${BACKEND_URL}${API_PREFIX}/twelvelabs/search_video`, {
		method: "POST",
		headers: {
...
export const generateResponse = async (
	...
	const response = await fetch(`${BACKEND_URL}${API_PREFIX}/conversation/generate-response`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
		},
	...
	if (!response.ok) {
		throw new Error(`Generate response failed: ${response.status} ${response.statusText}`);
	}
...
export const queryMemoryService = async ({
	...
	try {
		...
		const responseData = await generateResponse(userPrompt, indexedAssetId, analysisText);
